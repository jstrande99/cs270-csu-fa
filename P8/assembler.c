...
line_info_t* asm_pass_one (const char* asm_file_name,
	...
	FILE* sfile = fopen(asm_file_name, "r"); 
	...
	while(fgets(line, MAX_LINE_LENGTH, sfile) != NULL){ 
		current = tokenize_lc3_line(line);
		srcLineNum++;
		...
		if(util_is_valid_lable(token)){ 
			symbol_add(sym, token, currAddr);
			token = next_token();
		}
